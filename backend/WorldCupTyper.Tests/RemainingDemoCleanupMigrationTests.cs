...
public sealed class RemainingDemoCleanupMigrationTests
{
    [Fact]
    public void RemoveRemainingDemoMatches_ShouldDeleteEveryNullExternalIdMatch()
    {
        var migrationType = typeof(WorldCupTyperDbContext).Assembly
            .GetTypes()
            .SingleOrDefault(type => typeof(Migration).IsAssignableFrom(type)
                && type.Name.Contains("RemoveRemainingDemoMatches", StringComparison.Ordinal));
        ...
        sql.Should().NotContain("""match."MatchNumber" IN (1, 2, 3, 4, 5)""");
